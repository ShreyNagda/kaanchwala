...
import { LENS_ADD_ONS, type Product, type Variant, type LensAddOn } from '@/lib/types'
import { ShoppingBag, Check, Sparkles, ArrowLeft, Truck, Shield, Glasses, Sun, Eye } from 'lucide-react'
...
export function ProductDetail({ product, variants }: ProductDetailProps) {
 ...
 <img
 src={selectedVariant?.image_url || product.image_urls[0]}
 alt={product.title}
 className="h-full w-full object-cover"
 />
 ) : (
 <div className="h-full w-full flex items-center justify-center text-muted-foreground">
 {product.category === 'sunglasses' ? <Sun className="h-24 w-24" /> : product.category === 'contact_lenses' ? <Eye className="h-24 w-24" /> : <Glasses className="h-24 w-24" />}
 </div>
 )}
 {product.is_premium && (
 ...
 <button
 key={idx}
 className="h-20 w-20 rounded-lg bg-muted overflow-hidden border-2 border-transparent hover:border-accent transition-colors flex-shrink-0"
 >
 ...
 <span className="badge badge-destructive">{savingsPercent}% OFF</span>
 </>
 )}
 </div>
 ...
 Color: <span className="text-accent">{selectedVariant?.color}</span>
 </h3>
 ...
 {showLensAddOns && (
 <div>
 <h3 className="text-sm font-semibold text-foreground mb-3">Lens Add-Ons</h3>
 ...
 {variants.length > 0 && !selectedVariant?.is_in_stock ? 'Out of Stock' : 'Add to Cart'}
 </button>
